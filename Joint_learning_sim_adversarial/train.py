...
import cv2
from model.ResNet_models import Generator, FCDiscriminator_COD, FCDiscriminator_SOD
...
from save_to_temp import visualize_cod_gt,visualize_cod_prediction,\
    ...
    visualize_cod_edge,visualize_sal_edge, visualize_cod_dis, visualize_sod_dis
...
parser.add_argument('--lr_dis_sod', type=float, default=1e-5, help='learning rate for generator')
parser.add_argument('--lr_dis_cod', type=float, default=1e-5, help='learning rate for generator')
...
discrminator_sod = FCDiscriminator_SOD()
discrminator_sod.cuda()
discrminator_sod_params = discrminator_sod.parameters()
discrminator_sod_optimizer = torch.optim.Adam(discrminator_sod_params, opt.lr_dis_sod)

discrminator_cod = FCDiscriminator_COD()
discrminator_cod.cuda()
...
sim_train_loader = get_loader_val(sim_img_root, batchsize=opt.batchsize, trainsize=opt.trainsize)

sal_train_iter = iter(train_loader)
sal_it = 0
cod_train_iter = iter(cod_train_loader)
cod_it = 0
sim_train_iter = iter(sim_train_loader)
sim_it = 0
...
for i in range(1,opt.train_iters+1):
    ...
    if (i > opt.start_sim) * (i < opt.stop_sim):

        # shared_optimizer.step() twice
        if sim_it >= len(sim_train_loader):
            sim_train_iter = iter(sim_train_loader)
            sim_it = 0

        sim_pack = sim_train_iter.next()
        sim_imgs = sim_pack
        sim_imgs = Variable(sim_imgs)
        sim_imgs = sim_imgs.cuda()
        ...
        if sim_imgs.shape[0] != sal_imgs.shape[0]:
            sim_train_iter = iter(sim_train_loader)
            sim_it = 0
        ...
        discrminator_sod_optimizer.zero_grad()
        sal_init,sal_ref, sim_loss = generator.forward(sal_imgs,sim_imgs,is_sal=True)

        Dis_output1 = discrminator_sod(sal_imgs, torch.sigmoid(sal_init.detach()))
        Dis_output1 = F.upsample(Dis_output1, size=(sal_imgs.shape[2], sal_imgs.shape[3]), mode='bilinear',
                                align_corners=True)
        loss_dis_output1 = CE(Dis_output1, make_Dis_label(gt_label, sal_gts))

        Dis_output2 = discrminator_sod(sal_imgs, torch.sigmoid(sal_ref.detach()))
        Dis_output2 = F.upsample(Dis_output2, size=(sal_imgs.shape[2], sal_imgs.shape[3]), mode='bilinear',
                                 align_corners=True)
        sal_dis = Dis_output2
        loss_dis_output2 = CE(Dis_output2, make_Dis_label(gt_label, sal_gts))

        loss_dis_output = 0.5*(loss_dis_output1 + loss_dis_output2)
        # init loss
        ...
        # total loss
        sal_loss=sal_structure_loss1+sal_structure_loss2+sim_loss + 0.1*loss_dis_output
        ...
        Dis_output2 = F.upsample(Dis_output2, size=(sal_imgs.shape[2], sal_imgs.shape[3]), mode='bilinear',
                                 align_corners=True)
        Dis_target = discrminator_sod(sal_imgs, sal_gts)
        Dis_target = F.upsample(Dis_target, size=(sal_imgs.shape[2], sal_imgs.shape[3]), mode='bilinear',
                                align_corners=True)

        loss_dis_output = 0.5*(CE(Dis_output1, make_Dis_label(pred_label, sal_gts))+CE(Dis_output2, make_Dis_label(pred_label, sal_gts)))
        loss_dis_target = CE(Dis_target, make_Dis_label(gt_label, sal_gts))
        dis_loss = 0.5 * (loss_dis_output + loss_dis_target)
        dis_loss.backward()
        discrminator_sod_optimizer.step()
        ...
        generator_optimizer.zero_grad()
        discrminator_cod_optimizer.zero_grad()
        cod_init, cod_ref = generator.forward(cod_imgs, is_sal=False)

        Dis_output1 = discrminator_cod(cod_imgs, torch.sigmoid(cod_init.detach()))
        Dis_output1 = F.upsample(Dis_output1, size=(cod_imgs.shape[2], cod_imgs.shape[3]), mode='bilinear',
                                 align_corners=True)
        loss_dis_output1 = CE(Dis_output1, make_Dis_label(gt_label, cod_gts))

        Dis_output2 = discrminator_cod(cod_imgs, torch.sigmoid(cod_ref.detach()))
        Dis_output2 = F.upsample(Dis_output2, size=(cod_imgs.shape[2], cod_imgs.shape[3]), mode='bilinear',
                                 align_corners=True)
        cod_dis = Dis_output2
        loss_dis_output2 = CE(Dis_output2, make_Dis_label(gt_label, cod_gts))
        ...
        # total loss
        cod_loss = cod_ce_loss1 + cod_ce_loss2 + 0.1*loss_dis_output
        ...
        Dis_output2 = F.upsample(Dis_output2, size=(cod_imgs.shape[2], cod_imgs.shape[3]), mode='bilinear',
                                 align_corners=True)
        Dis_target = discrminator_cod(cod_imgs, cod_gts)
        Dis_target = F.upsample(Dis_target, size=(cod_imgs.shape[2], cod_imgs.shape[3]), mode='bilinear',
                                align_corners=True)

        loss_dis_output = 0.5 * (CE(Dis_output1, make_Dis_label(pred_label, cod_gts)) + CE(Dis_output2,
                                                                                            make_Dis_label(pred_label,
                                                                                                           cod_gts)))
        loss_dis_target = CE(Dis_target, make_Dis_label(gt_label, cod_gts))
        dis_loss = 0.5 * (loss_dis_output + loss_dis_target)
        dis_loss.backward()
        discrminator_cod_optimizer.step()
        ...
        generator_optimizer.zero_grad()
        discrminator_sod_optimizer.zero_grad()
        sal_init, sal_ref = generator.forward(sal_imgs, is_sal=True)
        ...
        # total loss
        sal_loss = sal_structure_loss1 + sal_structure_loss2 + 0.1*loss_dis_output
        ...
        loss_dis_output = 0.5 * (CE(Dis_output1, make_Dis_label(pred_label, sal_gts)) + CE(Dis_output2,
                                                                                           make_Dis_label(pred_label,
                                                                                                          sal_gts)))
        loss_dis_target = CE(Dis_target, make_Dis_label(gt_label, sal_gts))
        dis_loss = 0.5 * (loss_dis_output + loss_dis_target)
    ...
    if i % 10 == 0 or i == len(train_loader) or i == len(cod_train_loader):
        ...
        visualize_sod_dis(torch.sigmoid(sal_dis))
        visualize_cod_dis(torch.sigmoid(cod_dis))
